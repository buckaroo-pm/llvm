...
genrule(
  ...
  cmd = ' && '.join([
    'cd $TMP', 
    'cmake $SRCDIR -DLLVM_ENABLE_ZLIB=OFF -DLLVM_INCLUDE_GO_TESTS=OFF -DLLVM_ENABLE_BINDINGS=OFF -DLLVM_ENABLE_RTTI=OFF -DLLVM_INCLUDE_TOOLS=OFF -DLLVM_INCLUDE_DOCS=OFF -DLLVM_INCLUDE_TESTS=OFF -DLLVM_USE_PERF=OFF -DLLVM_INCLUDE_EXAMPLES=OFF -DLLVM_APPEND_VC_REV=OFF -DCMAKE_BUILD_TYPE=' + cmake_build_type, 
